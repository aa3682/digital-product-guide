...
## Hard rules
1. No platform, payment processor, marketplace, or brand names in site content or project outputs: everything under content/, page copy, titles, slugs, frontmatter, metadata, alt text, README, code comments, branch names, human-written commit subjects and bodies, and human-written PR text. Four things are exempt: (a) tooling provenance, meaning the upstream source reference in the Tooling section; (b) output generated by a coding tool rather than written by a person: commit trailers, PR footers, and the branch name a coding-tool session assigns, such as Co-Authored-By, Claude-Session, and "Generated with Claude Code"; (c) in a report the owner asked for, such as an audit or a name scan, inside a PR description: names quoted as findings, and the names and versions of tools used to produce the report where they are needed to reproduce its method (the PR title and the summary above the report stay under the rule); (d) vendored third-party tooling files under .claude/skills/, kept unedited. One exception for site content: /tools/this-years-figures may name a platform, and only to source that platform's own fee from the platform's own published fee or terms page. Everywhere else use the plain category: storefront, payment processor, email service, course platform, marketplace.
...
- The rule 7 overrides in app/globals.css depend on these names from the theme's rendered output: the focus-ring classes x:nextra-focus and x:focus-visible:nextra-focus; the class light on <html>; the drawer class nextra-mobile-nav; and the menu-icon state .nextra-hamburger svg.open. After any framework or theme upgrade, confirm each one still appears in the rendered page. Two of them never appear in the build output: a script adds light after the page loads, and x:nextra-focus is present only while an element has keyboard focus. Check both in a live page, not by searching the build. The drawer rule fails closed: if svg.open is renamed or removed, the rule matches all the time, and the mobile menu opens blank. Check it first, at 390px: open the menu and confirm its links show, then close it and confirm Tab does not reach them.
