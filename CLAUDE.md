...
## Hard rules
1. No platform, payment processor, marketplace, or brand names in site content or project outputs: everything under content/, page copy, titles, slugs, frontmatter, metadata, alt text, README, code comments, branch names, human-written commit subjects and bodies, and human-written PR text. Two things are exempt: (a) tooling provenance, meaning the upstream source reference in the Tooling section; (b) tool-generated commit trailers and PR footers, such as Co-Authored-By, Claude-Session, and "Generated with Claude Code". One exception for site content: /tools/this-years-figures may name a platform, and only to source that platform's own fee from the platform's own published fee or terms page. Everywhere else use the plain category: storefront, payment processor, email service, course platform, marketplace.
...
- After any upgrade of the site framework or theme, re-measure focus indicators and every CSS override written under the accessibility exception in Hard rules, rule 7, because those overrides may depend on the theme's internal class or token names.
